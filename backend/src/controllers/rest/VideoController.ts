...
export class VideoController {
  static async saveVideo(req: Request, res: Response): Promise<any> {
    const { id: _id }: { id: string } = jwtTokenUtilities(req);
    try {
      await User.updateOne({ _id }, { $push: { videos: req.params.imdbID } });
      return res.status(201).send(await returnSelectedVideos(_id));
    } catch (error) {
      throw error;
    }
  }
  static async deleteVideo(req: Request, res: Response): Promise<any> {
